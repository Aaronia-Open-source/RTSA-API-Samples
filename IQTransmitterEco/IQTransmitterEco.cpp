...
void streamIQ(AARTSAAPI_Device d)
{
	...
	for (int i = 0; i < NumPackets; i++)
	{
		...
		AARTSAAPI_GetMasterStreamTime(&d, startTime);
		while (startTime + 0.5 < packet.startTime)
		{
			std::this_thread::sleep_for( std::chrono::milliseconds(int(1000 * (packet.startTime - startTime - 0.45))));
			AARTSAAPI_GetMasterStreamTime(&d, startTime);
		}
		...
		if ((res = AARTSAAPI_Open(&h)) == AARTSAAPI_OK)
		{
			...
			if ((res = AARTSAAPI_RescanDevices(&h, 2000)) == AARTSAAPI_OK)
			{
				...
				if ((res = AARTSAAPI_EnumDevice(&h, L"spectranv6eco", 0, &dinfo)) == AARTSAAPI_OK)
				{
					...
					if ((res = AARTSAAPI_OpenDevice(&h, &d, L"spectranv6eco/iqtransmitter", dinfo.serialNumber)) == AARTSAAPI_OK)
					{
