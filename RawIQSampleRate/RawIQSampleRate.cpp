...
void streamIQ( AARTSAAPI_Device d )
{
    ...
    uint64_t startTicks = getCurrentMillis();
    int numPackets = 0;
    double prevTime;
    ...
    for ( ;; )
    {
        ...
        if ( res == AARTSAAPI_OK )
        {
            ...
            {
                uint64_t timeTicks = getCurrentMillis();
                double rate = double( numSamples ) / double( timeTicks - startTicks ) * 1000;
                std::wcout << "Samples : " << numSamples << " Millis : " << timeTicks - startTicks << " Rate " << std::setprecision( 12 ) << rate << " err " << std::setprecision( 6 ) << ( rate - 92.16e6 ) / 92.16e4 << "%"
                           << " (" << 2000.0 / ( timeTicks - startTicks ) << "%)" << std::endl;
            }
