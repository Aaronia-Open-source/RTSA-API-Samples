		...
		if ((res = AARTSAAPI_Open(&h)) == AARTSAAPI_OK)
		{
			...
			if ((res = AARTSAAPI_RescanDevices(&h, 2000)) == AARTSAAPI_OK)
			{
				...
				if ((res = AARTSAAPI_EnumDevice(&h, L"spectranv6eco", 0, &dinfo)) == AARTSAAPI_OK)
				{
					...
					if ((res = AARTSAAPI_OpenDevice(&h, &d, L"spectranv6eco/sweepsa", dinfo.serialNumber)) == AARTSAAPI_OK)
					{
						// Begin configuration, get root of configuration tree

						AARTSAAPI_Config		config, root;
						AARTSAAPI_ConfigInfo	cinfo;
						...
						if (AARTSAAPI_ConfigRoot(&d, &root) == AARTSAAPI_OK)
						{
							...
							if (AARTSAAPI_ConfigFind(&d, &root, &config, L"main/startfreq") == AARTSAAPI_OK)
							{
								...
								std::wcerr << "StartFreq:" << cinfo.minValue << " - " << cinfo.maxValue << std::endl;
								AARTSAAPI_ConfigSetFloat(&d, &config, cinfo.minValue);
							}
							...
							{
								AARTSAAPI_ConfigGetInfo(&d, &config, &cinfo);
								std::wcerr << "StopFreq :" << cinfo.minValue << " - " << cinfo.maxValue << std::endl;
								AARTSAAPI_ConfigSetFloat(&d, &config, cinfo.maxValue);
							}
